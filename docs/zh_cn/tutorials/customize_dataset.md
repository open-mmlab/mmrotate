...
要支持新的数据格式，您可以将它们转换为现有的格式（DATA 格式）。您可以选择离线（在通过脚本训练之前）或在线（实施新数据集并在训练时进行转换）进行转换。
在 MMRotate 中，我们建议将数据转换为 DATA 格式并离线进行转换，如此您只需在数据转换后修改 config 的数据注释路径和类别即可。


### 将新数据格式重构为现有格式


最简单的方法是将数据集转换为现有数据集格式 (DATA)
...
在数据预处理之后，用户可以通过两个步骤来训练具有现有格式（例如 DATA 格式）的自定义新数据集：
...
假设您的自定义数据集是 DATA 格式，请确保您在自定义数据集中具有正确的注释：
