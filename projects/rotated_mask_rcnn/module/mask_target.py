...
def mask_target_single(pos_proposals, pos_assigned_gt_inds, gt_masks, cfg):
    ...
    if num_pos > 0:
        pos_proposals = rbox2hbox(pos_proposals.tensor)  # convert to hbox
        proposals_np = pos_proposals.cpu().numpy()
        maxh, maxw = gt_masks.height, gt_masks.width
        proposals_np[:, [0, 2]] = np.clip(proposals_np[:, [0, 2]], 0, maxw)
        proposals_np[:, [1, 3]] = np.clip(proposals_np[:, [1, 3]], 0, maxh)
        # proposals_np[:, 2] = np.clip(proposals_np[:, 2], 0, maxw)
        # proposals_np[:, 3] = np.clip(proposals_np[:, 3], 0, maxh)
        ...
        # resized_masks = []
        # for i in range(len(proposals_np)):
        #     mask = gt_masks[pos_assigned_gt_inds[i]]
        #     bbox = proposals_np[i, :]
        #     # x1, y1, x2, y2 = bbox
        #     x, y, w, h, t = bbox
        #     w = np.maximum(w, 1)
        ...
        # mask_targets = PlygonMasks(resized_masks, *out_shape)
