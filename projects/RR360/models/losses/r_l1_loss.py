...
def rl1_loss(pred: Tensor, target: Tensor) -> Tensor:
    ...
    pred_part2 = torch.where(
        pred_part2 > torch.tensor(np.pi, dtype=torch.float).cuda(),
        torch.tensor(np.pi, dtype=torch.float).cuda(), pred_part2)
    pred_part2 = torch.where(
        pred_part2 < -torch.tensor(np.pi, dtype=torch.float).cuda(),
        torch.tensor(np.pi, dtype=torch.float).cuda(), pred_part2)
    ...
    loss_part2 = torch.where(loss_part2_1 > np.pi, 2 * np.pi - loss_part2_1,
                             loss_part2_1)
