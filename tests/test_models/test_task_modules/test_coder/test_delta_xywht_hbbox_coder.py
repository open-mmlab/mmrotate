...
from mmrotate.core.bbox.coder import DeltaXYWHTHBBoxCoder
...
class TestDeltaBboxCoder(TestCase):
    ...
    def test_encode(self):
        ...
        expected_encode_bboxes = torch.Tensor([[-0.5000, -0.5000,  0.0000,  0.0000, -1.5708],
        [-0.5000, -0.5000, -0.6931, -0.6931, -1.4708],
        [-0.5000, -0.5000, -1.6094, -1.6094, -1.4708],
        [-0.5000, -0.5000,  0.0000,  0.0000, -1.2708]])

        out = coder.encode(
            HorizontalBoxes(proposals), RotatedBoxes(gt))
