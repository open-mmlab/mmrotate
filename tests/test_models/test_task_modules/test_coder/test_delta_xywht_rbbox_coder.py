...
def test_delta_bbox_coder():
    ...
    assert expected_decode_bboxes.allclose(out.tensor, atol=1e-04)
    out = coder.decode(
        RotatedBoxes(rois), deltas, max_shape=torch.Tensor((32, 32))).tensor
    ...
    assert out.allclose(batch_out.tensor)
