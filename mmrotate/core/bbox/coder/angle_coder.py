...
class PSCCoder(BaseBBoxCoder):
    ...
    def encode(self, angle_targets):
        ...
        phase_shift_targets = tuple(torch.cos(phase_targets + 2 * torch.pi * x / self.num_step) for x in range(self.num_step))
    ...
    def decode(self, angle_preds):
        ...
        phase_sin = torch.sum(angle_preds[:, 0:self.num_step] * self.coef_sin, dim=-1)
        phase_cos = torch.sum(angle_preds[:, 0:self.num_step] * self.coef_cos, dim=-1)
        phase_mod = phase_cos ** 2 + phase_sin ** 2
        phase = -torch.atan2(phase_sin, phase_cos)  # In range [-pi,pi)

        if self.dual_freq:
            phase_sin = torch.sum(angle_preds[:, self.num_step:(2 * self.num_step)] * self.coef_sin, dim=-1)
            phase_cos = torch.sum(angle_preds[:, self.num_step:(2 * self.num_step)] * self.coef_cos, dim=-1)
            phase_mod = phase_cos ** 2 + phase_sin ** 2
            phase2 = -torch.atan2(phase_sin, phase_cos) / 2

            # Phase unwarpping, dual freq mixing
            idx = torch.cos(phase) * torch.cos(phase2) + torch.sin(phase) * torch.sin(phase2) < 0  # Angle between phase and phase2 is obtuse angle
            phase2[idx] = phase2[idx] % (2 * torch.pi) - torch.pi  # Add pi to phase2 and keep it in range [-pi,pi)
            phase = phase2

        phase[phase_mod < self.thr_mod] *= 0  # Set the angle of isotropic objects to zero
        angle_pred = phase / 2
        return angle_pred
