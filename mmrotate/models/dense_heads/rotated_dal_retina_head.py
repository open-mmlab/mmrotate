...
class RotatedDalHead(RotatedRetinaHead):
    ...
    def _get_targets_single(self,
        ...
        bbox_preds = flat_bbox_preds.reshape(-1, 5)[inside_flags, :]

        decode_bbox_preds = self.bbox_coder.decode(anchors, bbox_preds)

        epoch_info = tuple((self.epoch - 1, self.max_epoch - 1))
        # print(self.epoch, self.max_epoch)
        if self.assign_by_circumhbbox is not None:
            gt_bboxes_assign = obb2hbb(gt_bboxes, self.assign_by_circumhbbox)
            assign_result = self.assigner.assign(
                anchors, decode_bbox_preds, gt_bboxes_assign, epoch_info,
                gt_bboxes_ignore, None if self.sampling else gt_labels)
        else:
            assign_result = self.assigner.assign(
                anchors, decode_bbox_preds, gt_bboxes, epoch_info,
                gt_bboxes_ignore, None if self.sampling else gt_labels)

        sampling_result = self.sampler.sample(assign_result, anchors,
                                              gt_bboxes)
        bbox_weights = assign_result.get_extra_property('box_weights')
        label_weights = assign_result.get_extra_property('label_weights')
    ...
    def get_targets(self,
        ...
        concat_bbox_preds_list = levels_to_images(bbox_preds_list)
